...
// which is required for Drizzle `relations` on the `profileTable` to work properly (see https://orm.drizzle.team/docs/relations),
// but then if we tried to `drizzle-kit generate && drizzle-kit migrate` with `drizzle.config.ts` having
// `schema: './src/lib/db/schema.ts'`, then we would get an error about trying to modify the `authUsers` table under the
// `auth` schema in Supabase (we are not supposed to touch that table/schema).
//
